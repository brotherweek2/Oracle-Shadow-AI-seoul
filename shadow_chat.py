...
[답변 원칙]
1. 데이터에 있는 내용(회피/의존 축분해 수치·진단된 한계·need·이식후보 제도 등)은 구체적 근거를 들어 쉽게 설명한다.
...
6. 담당자가 이해하기 쉽게, 그러나 정확하게. 답 끝에 가끔 "더 궁금한 점 있으면 편하게 물어보세요" 식으로 추가 질문을 유도해도 좋다.
...
def build_chat_context(cur, gu):
    ...
    slim = OrderedDict()
    for k in ("quadrant", "avoidance", "dependency", "회피분해", "의존분해"):
